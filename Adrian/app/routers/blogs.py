...
from .. import oauthour2, schemas, models, utils
...
def create_blog(blog: schemas.BlogCreate, db: Session = Depends(get_db), current_user = Depends(oauthour2.get_current_user)):
...
def edit_blog(id: int, blog: schemas.BlogCreate, db: Session = Depends(get_db), current_user = Depends(oauthour2.get_current_user)):
...
def delete_blog(id:int, db: Session = Depends(get_db), current_user = Depends(oauthour2.get_current_user)):
    blog = db.query(models.Blog).filter(models.Blog.id == id)
    if not blog.first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Blog with id {id} not found")
